...
try {
    ...
    // Google Drive Setup
    if (process.env.GOOGLE_CREDENTIALS && process.env.GOOGLE_CREDENTIALS.includes('"private_key"')) {
        try {
            const credentials = JSON.parse(process.env.GOOGLE_CREDENTIALS);
            if (credentials.private_key && !credentials.private_key.includes('XXX')) {
                ...
                console.warn('⚠️  Google Drive credentials are placeholder values, skipping initialization');
            }
        } catch (error) {
            console.warn('⚠️  Google Drive initialization failed:', error.message);
        }
    } else {
        console.warn('⚠️  GOOGLE_CREDENTIALS environment variable not set or invalid. Google Drive features will be disabled.');
    }
} catch (error) {
