...
async function createBusinessResourceEntry(formData, googleDriveFolderId, contactId) {
    if (!notion || !googleDriveFolderId) {
        console.warn('Notion or Google Drive not configured for Business Resources');
        return null;
    }

    try {
        const folderUrl = `https://drive.google.com/drive/folders/${googleDriveFolderId}`;
        const resourceName = `Gutachten ${formData.auftraggeber?.kennzeichen} - ${formData.auftraggeber?.name}`;

        console.log('📋 Creating Business Resource entry:', resourceName);

        const resourceData = {
            parent: {
                database_id: '25b5e4b8-4c44-8133-ba41-f551dd68717b' // Business Resources Database
            },
            properties: {
                'Name': {
                    title: [
        ...
        const businessResource = await notion.pages.create(resourceData);
        console.log('✅ Business Resource created:', businessResource.id);
        
        return businessResource;
    } catch (error) {
        console.error('Business Resource creation error:', error);
        throw new Error('Fehler beim Erstellen des Business Resource Eintrags');
    }
}
...
app.post('/api/submit-gutachten', async (req, res) => {
    try {
        ...
        let pdfBuffer;
        try {
            pdfBuffer = await generateGutachtenPDF(formData);
            console.log('✅ PDF generated, size:', pdfBuffer.length);
        } catch (pdfError) {
            console.error('❌ PDF generation failed:', pdfError.message);
            // Use placeholder PDF if generation fails
            pdfBuffer = Buffer.from('PDF generation failed - placeholder used');
            console.log('⚠️ Using placeholder PDF due to generation error');
        }
        ...
        console.log('🔍 Step 6: Creating Business Resources entry...');
        // Create Business Resources entry with Google Drive link
        let businessResourceId = null;
        if (folderId && notionContact?.id) {
            try {
                const businessResource = await createBusinessResourceEntry(formData, folderId, notionContact.id);
                businessResourceId = businessResource?.id;
                console.log('📋 Business Resource created:', businessResourceId);
            } catch (resourceError) {
                console.error('Business Resource error (non-blocking):', resourceError.message);
            }
        }
...
// Graceful Shutdown Handlers (Railway requirement)
process.on('SIGTERM', () => {
